class SmartStore {
  constructor({
    ...
    this.worker = new Worker(
      this.createCode(initialState, reducers, selectors, effects, deps),
      { name }
    );
...
const getStringify = SmartStore.combineSelectors(
  getTotal,
  getDouble,
  (total, double) => "Result = " + (total + double)
);
...
async function Program() {
  const store = new SmartStore({
    ...
    selectors: { getTotal, getDouble, getSuffix, getStringify },
    broadcast: true,
    deps: ["lodash.js"],
  });
  const store2 = new SmartStore({
    name: "Test2",
    initialState: { total: 0 },
    reducers: [reducerA, reducerB],
